function init() {
  ...
  document.getElementsByClassName("gallery-image")[0].onload = function () {
    document.getElementsByClassName("gallery-control")[0].style.width = String(
      document.getElementsByClassName("gallery-image")[0].width
    ).concat("px");
    document.getElementsByClassName("gallery-control")[0].style.height = String(
      document.getElementsByClassName("gallery-image")[0].height
    ).concat("px");
  };

  document
    .querySelectorAll(".navbar_page a, .navbar_home a")
    .forEach((item) => {
      item.onclick = function () {
        scrollToTop();
      };
...
function build() {
  ...
  for (var i = 0; i < imagesLength; i++) {
    images[i].getElementsByTagName("img")[0].src = images[
      i
  ...
  if (!params.page || params.page == "home") {
    ...
    document.getElementsByClassName("navbar_page-container")[0].style.display =
      "none";
    document.getElementById("article_home").style.display = "block";
  } else {
    document.getElementsByClassName("navbar_home-container")[0].style.display =
      "none";
    if (window.innerWidth >= 1100) {
      document.getElementsByClassName(
      ...
      document
        .getElementsByClassName("navbar_page")[0]
        .getElementsByClassName(info.nav)[0].style.backgroundColor =
        "rgba(" + [254, 244, 224, 0.3].join(",") + ")";
      document
        .getElementsByClassName("navbar_page")[0]
        .getElementsByClassName(info.nav)[0].style.transition =
        "box-shadow 0.2s";
    }
...
function gallery(action) {
  if (action) {
    ...
    if (subtitle) {
      var caption = subtitle.parentElement.getElementsByTagName("figcaption")[0]
        .innerHTML;
      document.getElementsByClassName(
        "gallery-subtitle"
      )[0].innerHTML = caption;
    }
    document.getElementsByClassName("gallery-container")[0].style.display =
      "block";
  } else {
    genInfo();
    // document.getElementsByTagName("html")[0].style.overflowY = "scroll";
    document.getElementsByClassName("gallery-container")[0].style.display =
      "none";
    window.location.hash = decodeURIComponent(info.query).replace(
      "&image=".concat(params.image),
      ""
    );
  }
}
...
function onloadBuild() {
  function openPDF(e) {
    window.open(
      "documents/table/".concat(
        e.target._tooltip._content.replace(/ /g, "_"),
        ".pdf"
      )
    );
  }
  ...
  for (var i = 0; i < hidePagesLength; i++) {
    document.getElementById("article_".concat(hidePages[i])).style.visibility =
      "hidden";
    document.getElementById("article_".concat(hidePages[i])).style.display =
      "block";
  }
  ...
  var image = L.imageOverlay("images/zemljevid/zemljevid.jpg", bounds).addTo(
    map
  );
  ...
  for (var i = 0; i < markersLength; i++) {
    info = L.circle(
      [bounds[1][0] - markers[i].y - offset, markers[i].x + offset],
      {
        color: "blue",
        fillColor: "lightblue",
        fillOpacity: 0.5,
...
function switchTab(element, number) {
  ...
  for (var i = 0; i < tabsLength; i++) {
    if (number != i) {
      ...
      element.parentElement.parentElement.querySelectorAll(".tabs-content>*")[
        i
      ].style.display = "none";
    } else {
